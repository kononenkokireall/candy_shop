...
class DataBaseSession(BaseMiddleware):
    ...
    # Метод __call__ позволяет экземпляру класса DataBaseSession быть вызываемым,
    # что необходимо для реализации middleware в aiogram.
    # Параметры:
    # - handler: функция-обработчик, которая принимает объект TelegramObject и словарь данных,
    #   возвращая асинхронное (Awaitable) значение.
    # - event: объект события (например, сообщение), полученный от Telegram.
    # - data: словарь дополнительных данных, который передается между middleware.
    async def __call__(
            self,
        ...
        # Контекстный менеджер гарантирует, что сессия будет корректно закрыта после использования.
        async with self.session_pool() as session:
            # Добавляем созданную сессию в словарь данных под ключом 'session',
            # чтобы она была доступна для дальнейшей обработки в функции-обработчике.
            data['session'] = session
            # Вызываем переданный обработчик (handler) с объектом события и обновленным словарем данных,
