...
class Paginator:
    def __init__(self, array: list | tuple, page: int = 1, per_page: int = 1):
        ...
        self.pages = math.ceil(self.len / self.per_page)  # Общее количество страниц

        logger.info("Инициализация Paginator...")
        logger.debug(
            "Создан Paginator: всего элементов = %d, элементов на странице = %d, "
            "всего страниц = %d",
            self.len,
            self.per_page,
            self.pages,
    ...
    def __get_slice(self):
        ...
        """
        start = (self.page - 1) * self.per_page  # Индекс начала текущей страницы
        stop = start + self.per_page  # Индекс конца текущей страницы
        logger.debug("Получение данных для страницы %d (срез: %d:%d)", self.page, start, stop)
    ...
    def get_page(self):
        ...
        logger.debug("Данные страницы %d: %s", self.page, page_items)
        return page_items
    ...
    def has_next(self):
        ...
        Returns:
            bool | int: Номер следующей страницы или False, если следующей страницы нет.
        """
        if self.page < self.pages:
            logger.debug("Следующая страница существует: %d", self.page + 1)
            return self.page + 1
    ...
    def has_previous(self):
        ...
        Returns:
            bool | int: Номер предыдущей страницы или False, если предыдущей страницы нет.
        """
        if self.page > 1:
            logger.debug("Предыдущая страница существует: %d", self.page - 1)
            return self.page - 1
    ...
    def get_next(self):
        ...
        if self.page < self.pages:
            self.page += 1
            logger.info("Переход на следующую страницу: %d", self.page)
    ...
    def get_previous(self):
        ...
        if self.page > 1:
            self.page -= 1
            logger.info("Переход на предыдущую страницу: %d", self.page)
