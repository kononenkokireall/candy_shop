...
async def carts(
    ...
    try:
        if menu_name == "delete":
            # Удаляем товар и возвращаем новое состояние корзины
            cart_exists = await orm_full_remove_from_cart(session, user_id,
                                                          product_id)
            # Обновляем страницу, если это не первая страница и корзина пуста
            if page > 1 and not cart_exists:
                page -= 1

        elif menu_name == "decrement":
            # Уменьшаем количество товара и проверяем, остался ли товар
            is_item_in_cart = await orm_reduce_product_in_cart(session,
                                                               user_id,
                                                               product_id)
            # Если товар удален (is_item_in_cart=False) и это не первая страница
            if page > 1 and not is_item_in_cart:
                page -= 1

        elif menu_name == "increment":
            # Добавляем товар в корзину
            await orm_add_product_to_cart(session, user_id, product_id)
        ...
        logger.exception(f"Ошибка в carts(): {e}")
