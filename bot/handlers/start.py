...
from keyboards import create_main_menu_keyboard, create_catalog_keyboard, create_city_selection_keyboard, CITIES
from states import OrderProcess

# Импорты для интеграции каталога товаров
from data.catalog_data import PRODUCT_CATALOG # Список категорий каталога
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    try:
        ...
        if user_data.get("name") and user_data.get("city"):
...
async def user_registration(message: Message, state: FSMContext):
    ...
    if not is_valid_name(user_name):
        logger.warning(f"Некорректное имя от пользователя {message.from_user.id}:"
                       f" {user_name}.")
    ...
    logger.info(f"Имя {user_name} сохранено для пользователя"
...
async def user_select_city(message: Message, state: FSMContext):
    ...
    logger.info(f"Пользователь {message.from_user.id} выбирает город: {user_city}.")

    if user_city not in CITIES:
        logger.warning(f"Некорректный город от пользователя"
                       f" {message.from_user.id}: {user_city}.")
    ...
    user_data = await state.get_data() # Получаем обновленные данные
    logger.info(f"Город {user_city} сохранен для пользователя"
                f" {message.from_user.id}:")

    await message.answer(
        WELCOME_NEW_USER_MSG.format(user_name=user_data["name"],
                                    user_city=user_city),
        reply_markup=create_catalog_keyboard(categories=list(PRODUCT_CATALOG.values()))
    )
    logger.info(f"Пользователь {message.from_user.id} завершил регистрацию.")
...
async def cmd_change_city(message: Message, state: FSMContext):
    ...
    await state.set_state(OrderProcess.SelectCity)
    await message.answer(CHANGE_CITY_PROMPT,
                         reply_markup=create_city_selection_keyboard())