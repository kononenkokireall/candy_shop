...
from sqlalchemy import (
    ...
    BigInteger, func, Index, UniqueConstraint
)
...
class Base(DeclarativeBase):
    ...
    # При создании записи в колонке автоматически будет установлено текущее время.
...
class Banner(Base):
    ...
    # Название баннера. Ограничение unique гарантирует, что каждое название будет уникальным.
    ...
    # Ссылка для администратора, например, для редактирования или просмотра деталей баннера.
...
class Category(Base):
    ...
    __table_args__ = (Index('idx_category_name', 'name', unique=True),)
...
class Product(Base):
    ...
    __table_args__ = (Index('idx_product_category', 'category_id'),)
...
class User(Base):
    ...
    # Cascade 'all, delete-orphan' означает, что при удалении пользователя все связанные заказы будут удалены.
    # Lazy='selecting' обеспечивает оптимизированную загрузку связанных объектов.
...
class Cart(Base):
    __tablename__ = 'cart'
    # Ограничение UniqueConstraint обеспечивает, что для каждой пары (user_id, product_id)
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    # Внешний ключ для связи с пользователем. При удалении пользователя запись корзины удаляется.
    user_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey('user.user_id', ondelete='CASCADE'),
        nullable=False
    )
    # Внешний ключ для связи с товаром. При удалении товара запись корзины удаляется.
...
class Order(Base):
    ...
    # Связь с элементами заказа. Cascade 'all, delete-orphan' гарантирует, что при удалении заказа
    # все связанные элементы также удалятся.
    # Passive_deletes=True позволяет базе данных управлять удалением записей, если установлен ON DELETE CASCADE.
...
# Модель OrderItem представляет таблицу "order_item" – отдельные позиции заказа.
class OrderItem(Base):
    __tablename__ = 'order_item'
    # Индекс для ускорения поиска по полю product_id.
    __table_args__ = (Index('idx_order_item_product', 'product_id'),)

    # Первичный ключ записи позиции заказа.
    id: Mapped[int] = mapped_column(primary_key=True)
    # Внешний ключ для связи с заказом. При удалении заказа удаляются связанные позиции.
    order_id: Mapped[int] = mapped_column(
        ForeignKey('orders.id', ondelete='CASCADE'),
        nullable=False
    )
    # Внешний ключ для связи с товаром. При удалении товара устанавливается значение NULL.
